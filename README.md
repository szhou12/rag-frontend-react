...
### Example
#### Case: Add A New Column `publication_date` to Table `Upload` (Table Already Has Data/Rows)
1. Modify database model first: `models/upload.py`
```python
class UploadBase(SQLModel):
    filename: str = Field(max_length=255)
    author: str = Field(max_length=255)
    language: str = Field(max_length=10)
    publication_date: Optional[datetime] = None   # <-- NEW, nullable for now
```
2. Generate a new Alembic version
In terminal, cd to where `alembic.ini` is located. In this project, it is under `/backend/`.
```bash
...
5. Verification
    ...
    ```sql
    SELECT id, publication_date, date_added FROM upload ORDER BY date_added DESC LIMIT 5;
    +----------------------------------+------------------+---------------------+
    | id                               | publication_date | date_added          |
    +----------------------------------+------------------+---------------------+
    | someid01-c759-4f7c-bbf4-91267ddc | NULL             | 2025-11-19 08:20:17 |
    | someid02-6bf6-42d2-8ab7-e6c54c1d | NULL             | 2025-11-19 08:18:49 |
    | someid03-60de-4822-8a20-9ad93ac7 | NULL             | 2025-11-19 08:18:22 |
    | someid04-9bf0-4506-bfa7-3048a69e | NULL             | 2025-11-19 08:13:19 |
